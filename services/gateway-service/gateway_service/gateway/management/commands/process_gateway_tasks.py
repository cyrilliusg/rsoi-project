# Async retry worker for the Redis-backed task queue. Disabled because
# Redis is not deployed in k8s — the command idles to keep the pod
# healthy. Drop the gateway-worker deployment if you don't need it.
...
class Command(BaseCommand):
    help = "Process async gateway tasks from Redis queue (disabled — idles)"

    def handle(self, *args, **options):
        self.stdout.write(
            "Gateway task worker started but Redis is disabled — idling."
        )
        while True:
            time.sleep(3600)
